...
import { SURAHS, surahsInJuz, JUZ_START_PAGE } from "@/lib/quran";
import { useState } from "react";
import { toast } from "sonner";
import { Mic, Award, BookOpen, History, ListTree } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="tambah">
        <TabsList className="rounded-xl">
          <TabsTrigger value="tambah"><Mic className="mr-1 h-4 w-4" />Tambah Tasmi'</TabsTrigger>
          <TabsTrigger value="mushaf"><BookOpen className="mr-1 h-4 w-4" />Mushaf Digital</TabsTrigger>
          <TabsTrigger value="riwayat"><History className="mr-1 h-4 w-4" />Riwayat</TabsTrigger>
          <TabsTrigger value="juz"><ListTree className="mr-1 h-4 w-4" />Daftar Juz 1–30</TabsTrigger>
        </TabsList>
        <TabsContent value="tambah"><TambahTab /></TabsContent>
        <TabsContent value="mushaf"><Card className="card-fun"><CardContent className="pt-6"><MushafViewer /></CardContent></Card></TabsContent>
        <TabsContent value="riwayat"><RiwayatTab /></TabsContent>
        <TabsContent value="juz"><JuzTab /></TabsContent>
      </Tabs>
    </div>
  );
}
...
function TambahTab() {
  ...
  const [form, setForm] = useState<any>({
    ...
    surah_from: "Al-Fatihah", surah_to: "", juz: 1, tasmi_type: "q1",
    duration_min: 15, score: 85, status: "lancar", notes: "",
  });
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("tasmi_entries").insert({
        student_id: form.student_id, teacher_id: user!.id, date: form.date,
        surah_from: form.surah_from, surah_to: form.surah_to || null, juz: form.juz,
        tasmi_type: form.tasmi_type, duration_min: form.duration_min,
        score: form.score, status: form.status, notes: form.notes,
      } as any);
      ...
      <CardContent className="space-y-3">
        ...
        <div>
          ...
          <Select value={form.tasmi_type} onValueChange={(v) => setForm({ ...form, tasmi_type: v })}>
            ...
            <SelectContent>{TASMI_TYPES.map((t) => <SelectItem key={t.key} value={t.key}>{t.label}</SelectItem>)}</SelectContent>
          </Select>
        </div>
...
function RiwayatTab() {
  const { data: rows } = useQuery({
    ...
    queryFn: async () => (await supabase.from("tasmi_entries").select("*, students(full_name)").order("date", { ascending: false }).limit(100)).data ?? [],
  });
  return (
    <Card className="card-fun">
...
function JuzTab() {
  ...
  const cetakSertifikat = () => {
    if (!s) { toast.error("Pilih santri terlebih dahulu"); return; }
    generateSertifikatPDF(s, juz, 90);
  };
    ...
    <div className="space-y-4">
      <Card className="card-fun">
        <CardContent className="pt-6 flex flex-wrap gap-3 items-end">
          <div className="min-w-40">
            <Label>Santri (untuk sertifikat)</Label>
          ...
          <p className="text-xs text-muted-foreground max-w-md">
            Klik juz di bawah untuk melihat daftar surah, lalu tekan tombol <b>Sertifikat</b> untuk mencetak sertifikat penyelesaian juz.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-2 grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-10">
        {Array.from({ length: 30 }, (_, i) => i + 1).map((j) => (
          <button key={j} onClick={() => setJuz(j)}
            className={`rounded-xl border-2 py-2 text-sm font-bold transition ${
              juz === j ? "border-primary bg-primary text-primary-foreground shadow" : "border-border hover:border-primary/50"
            }`}>Juz {j}</button>
        ))}
      </div>
      ...
      <Card className="card-fun">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            <ListTree className="h-5 w-5 text-primary" /> Juz {juz} — {surahs.length} surah · mulai halaman {JUZ_START_PAGE[juz]}
          </CardTitle>
          <Button onClick={cetakSertifikat} className="rounded-xl bg-sun text-black hover:bg-sun/90">
            <Award className="mr-2 h-4 w-4" />Sertifikat Juz {juz}
          </Button>
        </CardHeader>
        <CardContent>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {surahs.map((s) => (
              <div key={s.no} className="flex items-center justify-between rounded-lg border bg-muted/20 px-3 py-2 text-sm">
                <div>
                  <div className="font-display font-bold">{s.no}. {s.name}</div>
                  <div className="text-xs text-muted-foreground">{s.ayat} ayat</div>
                </div>
                <span className="rounded-full bg-leaf/20 px-2 py-0.5 text-[10px] font-bold text-leaf">Juz {juz}</span>
              </div>
            ))}
            {surahs.length === 0 && <p className="text-sm text-muted-foreground">Belum ada data.</p>}
          </div>
        </CardContent>
      </Card>
    </div>
